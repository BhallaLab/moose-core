...
**
** Updated by Upi Bhalla 15 years later in Feb 2006, to use
** tabchannels for the Kca_mit_usb. I have a vague memory of someone
** having done this, but here goes again.
*/
...
function make_Kca_mit_usb
	...
	create tabchannel Kca_mit_usb
		setfield ^ Ek {EK} Gbar {360.0*SOMA_A} Ik 0 Gk 0 \
		Xpower 1 Ypower 0 Zpower 1 instant {INSTANTX}

	/*
	setupalpha Kca_mit_usb X \
		{1.0/0.027} 0 0 {EREST_ACT} -1 \
		1 0 0 1 1 \
		-size 3000 -range -0.1 0.05
		*/
	int XDIVS = 3000
	float XMIN = -0.1
	float XMAX = 0.05
	call Kca_mit_usb TABCREATE X {XDIVS} {XMIN} {XMAX}
	float x = {XMIN}
	float dx = {XMAX - XMIN} / {XDIVS}
	float y
    for (i = 0 ; i <= {XDIVS} ; i = i + 1)
		y = {exp {(x - {EREST_ACT})/0.027}}
		setfield Kca_mit_usb X_A->table[{i}] {y}
        setfield Kca_mit_usb X_B->table[{i}] 1.0
		x = x + dx
	end
	
	setupalpha Kca_mit_usb Z \
		{5.0e5*0.015} -5.0e5 -1.0 -0.015 -0.0013 \
		50 0 0 1 1 \
		-size 1000 -range 0.0 0.01
	xdivs = 1000
    for (i = 0 ; i <= {xdivs} ; i = i + 1)
         setfield Kca_mit_usb Z_B->table[{i}] 50.0
	end
		
    addfield Kca_mit_usb addmsg1
    setfield Kca_mit_usb addmsg1  "../Ca_mit_conc  . CONCEN Ca" 
end
