...
class SquidSetup(object):
    ...
    def setup_recording(self):
        ...
        moose.connect(self.cmd_table, 'requestData', self.clamp_ckt.vclamp, 'get_output')
        self.iclamp_table = moose.Table('/data/Iclamp')
        moose.connect(self.iclamp_table, 'requestData', self.clamp_ckt.iclamp, 'get_output')
        self.vclamp_table = moose.Table('/data/Vclamp')
        moose.connect(self.vclamp_table, 'requestData', self.clamp_ckt.pid, 'get_output')
        self.m_table = moose.Table('/data/m')
        moose.connect(self.m_table, 'requestData', self.squid_axon.Na_channel, 'get_X')
        self.h_table = moose.Table('/data/h')
        moose.connect(self.h_table, 'requestData', self.squid_axon.Na_channel, 'get_Y')
        self.n_table = moose.Table('/data/n')
        moose.connect(self.n_table, 'requestData', self.squid_axon.K_channel, 'get_X')
        self.ina_table = moose.Table('/data/INa')
        moose.connect(self.ina_table, 'requestData', self.squid_axon.Na_channel, 'get_Ik')
        self.ik_table = moose.Table('/data/IK')
        moose.connect(self.ik_table, 'requestData', self.squid_axon.K_channel, 'get_Ik')
