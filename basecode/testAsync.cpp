...
void testSendMsg()
{
    ...
    const DestFinfo* df =
        dynamic_cast<const DestFinfo*>(ac->findFinfo("setOutputValue"));
    assert(df != 0);
    ...
    for(unsigned int i = 0; i < size; ++i) {
        double temp = i + i * i;
        double val =
            reinterpret_cast<Arith*>(e2.element()->data(i))->getOutput();
...
void testCreateMsg()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        const SrcFinfo1<double>* sf =
            dynamic_cast<const SrcFinfo1<double>*>(f1);
        assert(sf != 0);
    ...
    /*
    for ( unsigned int i = 0; i < size; ++i )
        cout << i << "	" << reinterpret_cast< Neutral* >(e2.element()->data(
    i ))->getName() << endl;
...
void testStrSet()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        ...
        double val =
            reinterpret_cast<Arith*>(Eref(i2.element(), i).data())->getOutput();
...
void testSetGetDouble()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        ...
        assert(ret);
        assert(doubleEq(reinterpret_cast<IntFire*>(oid.data())->getVm(), temp));
    }
...
void testSetGetSynapse()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        ret = Field<double>::setVec(ObjId(syns, i), "delay", delay[i]);
        if(i > 0)
            assert(ret);
    }
...
void testSetGetVec()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        SimpleSynHandler* ssh =
            reinterpret_cast<SimpleSynHandler*>(i2.element()->data(i));
...
void testSendSpike()
{
    ...
    reinterpret_cast<IntFire*>(targetCell.data())
        ->process(targetCell.eref(), &p);
...
void testSparseMatrix2()
{
    ...
    vector<unsigned int> keepCols(init,
                                  init + sizeof(init) / sizeof(unsigned int));
...
void testSparseMsg()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        ObjId id(sshid, i);
        unsigned int numSyn = Field<unsigned int>::get(id, "numSynapse");
...
{
    static ValueFinfo<TestId, Id> id("id", "test", &TestId::setId,
                                     &TestId::getId);
    static Finfo* testIdFinfos[] = {&id};
    static Cinfo testIdCinfo(
        "TestIdRepeatAssignment", Neutral::initCinfo(), testIdFinfos,
        sizeof(testIdFinfos) / sizeof(Finfo*), new Dinfo<TestId>());
    return &testIdCinfo;
}
...
void testSetRepeat()
{
    ...
    for(unsigned int i = 0; i < size; ++i) {
        ret = Field<double>::setRepeat(ObjId(synapse, i), "delay", 123.0);
        assert(ret);
    }
    for(unsigned int i = 0; i < size; ++i) {
        vector<double> delay;
...
public:
    ...
    static const Cinfo* initCinfo()
    {
        static SharedFinfo shared("shared", "", sharedVec,
                                  sizeof(sharedVec) / sizeof(const Finfo*));
        static Finfo* testFinfos[] = {
            &shared,
        };

        static Dinfo<Test> dinfo;
        static Cinfo testCinfo("Test", 0, testFinfos,
                               sizeof(testFinfos) / sizeof(Finfo*), &dinfo);
...
void testSharedMsg()
{
    ...
    static DestFinfo d0("d0", "", new OpFunc0<Test>(&Test::handleS0));
    static DestFinfo d1("d1", "", new EpFunc1<Test, string>(&Test::handleS1));
    static DestFinfo d2("d2", "", new EpFunc2<Test, int, int>(&Test::handleS2));
...
void testConvVector()
{
    ...
    assert(sz ==
           1 + 2 + (strVec[2].length() + 8) / 8 + (strVec[3].length() + 8) / 8);
    assert(buf[0] == 4);
    assert(strcmp(reinterpret_cast<char*>(buf + 1), "one") == 0);

    tempBuf = buf;
    const vector<string>& tgtStr = Conv<vector<string>>::buf2val(&tempBuf);
...
void testConvVectorOfVectors()
{
    ...
    double expected[] = {6,  // Number of sub-vectors
                         0,  // No entries on first sub-vec
                         1, 1, 2, 2,  3, 3,  4,  5,  6,  4,
                         7, 8, 9, 10, 5, 11, 12, 13, 14, 15};
...
void testLookupSetGet()
{
    ...
    double ret = LookupField<unsigned int, double>::get(obj, "anyValue", 0);
...
void testFinfoFields()
{
    ...
    const FinfoWrapper synFinfo =
        SimpleSynHandler::initCinfo()->findFinfo("synapse");
    const FinfoWrapper procFinfo = IntFire::initCinfo()->findFinfo("proc");
    const FinfoWrapper processFinfo =
        IntFire::initCinfo()->findFinfo("process");
    ...
    const FinfoWrapper classNameFinfo =
        Neutral::initCinfo()->findFinfo("className");
...
void testCinfoFields()
{
    ...
    */
    ...
    assert(cinfo->getDestFinfo(6 + ndf) ==
           cinfo->findFinfo("setRefractoryPeriod"));
    assert(cinfo->getDestFinfo(7 + ndf) ==
           cinfo->findFinfo("getRefractoryPeriod"));
    ...
    assert(cinfo->getValueFinfo(3 + nvf) ==
...
void testCinfoElements()
{
    ...
    unsigned int n = Field<unsigned int>::get(intFireValueFinfoId, "numData");
    assert(n == 4);
...
void testMsgSrcDestFields()
{
    ...
    */
    ...
    assert(pairs[0].first ==
           dynamic_cast<SrcFinfo*>(Test::sharedVec[0])->getBindIndex());
    assert(pairs[0].second ==
           dynamic_cast<DestFinfo*>(Test::sharedVec[1])->getFid());

    assert(pairs[1].first ==
           dynamic_cast<SrcFinfo*>(Test::sharedVec[2])->getBindIndex());
    assert(pairs[1].second ==
           dynamic_cast<DestFinfo*>(Test::sharedVec[3])->getFid());

    assert(pairs[2].first ==
           dynamic_cast<SrcFinfo*>(Test::sharedVec[4])->getBindIndex());
    assert(pairs[2].second ==
           dynamic_cast<DestFinfo*>(Test::sharedVec[5])->getFid());
    ...
    vector<ObjId> tgt;
    vector<string> func;
    unsigned int numTgt = e1->getMsgTargetAndFunctions(
        0, dynamic_cast<SrcFinfo*>(Test::sharedVec[0]), tgt, func);
    ...
    numTgt = e2->getMsgTargetAndFunctions(
        0, dynamic_cast<SrcFinfo*>(Test::sharedVec[4]), tgt, func);
    assert(numTgt == tgt.size());
    assert(tgt.size() == 1);
    ...
    FuncId fid = static_cast<const DestFinfo*>(Test::sharedVec[5])->getFid();
    unsigned int numSrc =
        t2.element()->getMsgSourceAndSender(fid, source, sender);
    assert(numSrc == 1);
...
void testHopFunc()
{
    ...
    const char* c = reinterpret_cast<const char*>(buf + TgtInfo::headerSize);
