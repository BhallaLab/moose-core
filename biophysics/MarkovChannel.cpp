...
const Cinfo* MarkovChannel::initCinfo()
{
    ...
    static ValueFinfo<MarkovChannel, double> ligandconc("ligandConc",
        "Ligand concentration.", &MarkovChannel::setLigandConc,
        &MarkovChannel::getLigandConc);

    static ValueFinfo<MarkovChannel, double> vm(
        "Vm", "Membrane voltage.", &MarkovChannel::setVm, &MarkovChannel::getVm);

    static ValueFinfo<MarkovChannel, unsigned int> numstates("numStates",
        "The number of states that the channel can occupy.", &MarkovChannel::setNumStates,
    ...
    static ValueFinfo<MarkovChannel, unsigned int> numopenstates("numOpenStates",
        ...
        &MarkovChannel::setNumOpenStates, &MarkovChannel::getNumOpenStates);

    static ValueFinfo<MarkovChannel, vector<string>> labels("labels",
        "Labels for each state.", &MarkovChannel::setStateLabels,
        &MarkovChannel::getStateLabels);

    static ReadOnlyValueFinfo<MarkovChannel, vector<double>> state("state",
        "This is a row vector that contains the probabilities of finding the channel in "
        "each state.",
        &MarkovChannel::getState);

    static ValueFinfo<MarkovChannel, vector<double>> initialstate("initialState",
        "This is a row vector that contains the probabilities of finding the channel in "
        "each state at t = 0. The state of the channel is reset to this value during a "
        "call to reinit()",
        &MarkovChannel::setInitialState, &MarkovChannel::getInitialState);

    static ValueFinfo<MarkovChannel, vector<double>> gbar("gbar",
        "A row vector containing the conductance associated with each of the "
        "open/conducting states.",
        &MarkovChannel::setGbars, &MarkovChannel::getGbars);
    ...
    static string doc[] = {"Name", "MarkovChannel", "Author",
        "Vishaka Datta S, 2011, NCBS Bangalore", "Description",
        "MarkovChannel.\n"
    ...
    static Cinfo MarkovChannelCinfo("MarkovChannel", ChanBase::initCinfo(),
        MarkovChannelFinfos, sizeof(MarkovChannelFinfos) / sizeof(Finfo*), &dinfo, doc,
...
MarkovChannel::MarkovChannel() : g_(0), ligandConc_(0), numStates_(0), numOpenStates_(0)
{
    ;
}
