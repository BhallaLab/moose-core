...
void SymCompartment::innerInitProc( const Eref& e, ProcPtr p )
{
	...
	distalOut()->send( e, p->threadIndexInGroup, Ra_, Vm_ ); // to kids
	proximalOut()->send( e, p->threadIndexInGroup, Ra_, Vm_ ); // to parent
	cylinderOut()->send( e, p->threadIndexInGroup, 1e-6, Vm_ ); // to kids
}
