...
#if defined _LIBC && defined USE_NONOPTION_FLAGS
# define NONOPTION_P (argv[woptind][0] != '-' || argv[woptind][1] == '\0'       \
                      || (woptind < nonoption_flags_len                        \
                          && __getopt_nonoption_flags[woptind] == '1'))
#else
# define NONOPTION_P (argv[woptind][0] != '-' || argv[woptind][1] == '\0')
#endif
  ...
  if (nextchar == NULL || *nextchar == '\0')
    {
      ...
      if (woptind != argc && !strcmp (argv[woptind], "--"))
        {
          woptind++;
          ...
          if (print_errors)
            {
              ...
              free (buf);
#else
              fprintf (stderr, w_("%s: option `%s' is ambiguous\n"),
                       argv[0], argv[woptind]);
#endif
            }
...
#else
                      fprintf (stderr,
                               w_("%s: option `%s' requires an argument\n"),
              ...
              if (argv[woptind][1] == '-')
                {
                  ...
                  __asprintf (&buf, _("%s: unrecognized option `--%s'\n"),
                              argv[0], nextchar);
#else
                  fprintf (stderr, w_("%s: unrecognized option `--%s'\n"),
                           argv[0], nextchar);
#endif
                }
              else
...
#else
                  fprintf (stderr, w_("%s: unrecognized option `%c%s'\n"),
            ...
            if (posixly_correct)
              {
                ...
                __asprintf (&buf, _("%s: illegal option -- %c\n"),
                            argv[0], c);
#else
                fprintf (stderr, w_("%s: illegal option -- %c\n"), argv[0], c);
#endif
              }
            else
              {
                ...
                __asprintf (&buf, _("%s: invalid option -- %c\n"),
                            argv[0], c);
#else
                fprintf (stderr, w_("%s: invalid option -- %c\n"), argv[0], c);
#endif
              }
    ...
    if (temp[0] == 'W' && temp[1] == ';')
      {
            ...
            if (print_errors)
              {
                ...
                free (buf);
#else
                fprintf (stderr, w_("%s: option requires an argument -- %c\n"),
                         argv[0], c);
#endif
              }
            woptopt = c;
        ...
        for (nextchar = nameend = woptarg; *nameend && *nameend != '='; nameend++)
          /* Do nothing.  */ ;
            ...
            if (print_errors)
              {
                ...
                free (buf);
#else
                fprintf (stderr, w_("%s: option `-W %s' is ambiguous\n"),
                         argv[0], argv[woptind]);
#endif
              }
            nextchar += strlen (nextchar);
...
#else
                    fprintf (stderr,
                             w_("%s: option requires an argument -- %c\n"),
                             argv[0], c);
#endif
                  }
                woptopt = c;
...
int wgetopt (int argc, char * const * argv, const char * optstring)
{
