...
gnuplot_ctrl * gnuplot_init(void)
{
	...
	//putenv("DISPLAY=192.168.3.54:0.0");
        //fprintf(stderr, "cannot find DISPLAY variable: is it set?\n") ;
	//fprintf(stderr, "Setting DISPLAY variable to: %s \n", getenv("DISPLAY") ) ;
    }
...
)
{
    ...
    fpTmpFile = fopen("gnuplot_tmp", "w");
    if(fpTmpFile == NULL)
    {
	printf("\n ****** Error: Cannot open tmp file for gnuplot");
	return;
    }

    /* Write data to this file  */
    for (i=0 ; i<n; i++) {
        fprintf(fpTmpFile, "%g	%g\n", x[i], y[i]) ;
    }
    ...
    if( n > 0)
    {
	    if (title == NULL) {
        	...
        	sprintf(line, "%s \"%s\" title \"%s\" ", cmd, "gnuplot_tmp", title) ;
	    }
