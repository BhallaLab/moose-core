...
class KineticsWidget(QtGui.QWidget):
    def __init__(self,size,modelPath,parent=None):
        ...
        cmapFile = open(os.path.join(config.settings[config.KEY_COLORMAP_DIR], 'rainbow2.pkl'), 'rb')
        ...
        if self.xmax > self.xmin:
            self.xratio /= (self.xmax-self.xmin)        
        if self.ymax > self.ymin:
            self.yratio /= (self.ymax - self.ymin)
    ...
    def setupDisplayInfo(self, modelPath):
        ...
        for meshEntry in moose.wildcardFind(meshEntryWildcard):
            ...
            reacDisplayDict = kutils.getReactionDisplayData(meshEntry, self.colorMap)
            enzymeDisplayDict, poolDisplayDict = kutils.getPoolDisplayData(meshEntry, self.colorMap)
            self.meshEntryDisplay[meshEntry] = {'reaction': reacDisplayDict, 
