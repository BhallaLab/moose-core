...
class KineticsWidget(QtGui.QWidget):
    def __init__(self,size,modelPath,parent=None):
        ...
        print " \n \t srcdesconnection dict \n"
        for m,n in self.srcdesConnection.items():
            print m.path, m[0].class_
            for ii in n:
                print '->', ii
        print " End"
            ...
            for cmpt,itemlist in cmptMol.items():
                ...
                for item in itemlist:
                    ...
                    if item[0][0].class_ in ['ZombieMMenz', 'ZombieEnz', 'ZombieReac']:
                        ...
                        print "\t \t mooseId_Gtext HARSHA",type(item[0]),item[0],reItem
                        self.mooseId_GText[item[0]] = reItem
                    ...
                    elif item[0][0].class_ in ['ZombiePool', 'ZombieFuncPool', 'ZombieBufPool']:
                        print "*******************************************Itens",item[0].class_,item
                        if item[0][0].parent.class_ != 'ZombieEnz':
                            ...
                            pItem = RectCompt1(x,item[2]*(-yratio),w,h,self.mooseId_GText[item[0][0].parent.getId()],item[0])
                            textcolor = ''
                        ...
                        self.mooseId_GText[item[0]] = pItem
        print "Printing mooseId_GText dict values"
        ...
        #for p,q in self.mooseId_GText.items():
        #    print p,q
        print '&&&&&&'
        print self.mooseId_GText.keys()
        print '^^^^^^^^^^^^^^^^^^^^'
        # for k in self.srcdesConnection.keys():
        #     for k2 in self.mooseId_GText.keys():
        #         print '>' , k,type(k), k2, type(k2), k == k2
            
        
        for inn,out in self.srcdesConnection.items():
            # if ((len(filter(lambda x:isinstance(x,list), out))) != 0):
            print '555', inn, inn.class_
            print '666', out
            print self.mooseId_GText.keys()
            src = self.mooseId_GText[inn]
            for outitems in out:
                print 'OutItems:', outitems
                for entry in outitems:
                    print '@', type(entry[0]), entry[0]
                    for k in self.mooseId_GText.keys():
                        print k, k == entry[0]
                    des = self.mooseId_GText[entry[0]]
                    self.lineCord(src,des,entry[1])                    
        ...
        for zombieObj in searObject:
            ...
            if zombieObj != 'ZombieSumFunc':
                for items in wildcardFind(path):
                    ...
                    if (zombieObj == 'ZombieMMenz'):
                        for enzpar in items[0].getNeighbors('enzDest'):
                            sublist.append((enzpar,'t'))
                    #print "here in setupitem",items[0],type(items)
                ...
                for items in wildcardFind(path):
                    ...
                    funplist = []
                    nfunplist = []
                    print "items in zombiesum",items
                    for inpt in items[0].getNeighbors('input'):                        
                        inputlist.append((inpt,'st'))
                    for zfun in items[0].getNeighbors('output'): 
                        nfunplist.append(zfun)
                    nfunplist = list(set(nfunplist))
                    print 'nfunplist:', nfunplist
                    print 'inputlist', inputlist
                    if len(nfunplist) == 0:
                        continue
                    if(len(nfunplist) > 1): 
                        print "SumFunPool has multiple Funpool"
                    else:                        
                        print '$$ key:', nfunplist[0]
                        print '$$ inputlist', inputlist
                        
                        cntDict[nfunplist[0]] = (inputlist, [])
                        # for el in funplist:
                        #     if el == nfunplist[0]:
    ...
    def setupCompt_Coord (self,filePath,mobject_Cord):
        ...
        for meshEnt in wildcardFind(cPath):
            ...
            for reitem in Neutral(meshEnt).getNeighbors('remeshReacs'):
                print '1111111111', reitem, reitem.class_
            ...
            for mitem in Neutral(meshEnt).getNeighbors('remesh'):
                print '0000000000', mitem, mitem.class_
                if isinstance(mitem.class_, tuple):
                    print type(mitem)
