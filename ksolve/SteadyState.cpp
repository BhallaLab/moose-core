/***
 *       Filename:  SteadyState.cpp
 *
 *    Description:  Steady state solver. Por of ../ksolve/SteadyStateGsl.cpp
 *    using Eigen.
 *
 *         Author:  Dilawar Singh <dilawars@ncbs.res.in>
 *   Organization:  NCBS Bangalore
 *
 * This program computes a steady-state value for a reaction system.
 * It uses Eigen library.
 *
 * It finds the ss_ value closest to the initial conditions.
 *
...
void ss_func(const column_type_& x, void* params, column_type_& f);
...
const Cinfo* SteadyState::initCinfo()
{
    ...
    // Field definitions
    ///////////////////////////////////////////////////////
    static ValueFinfo<SteadyState, Id> stoich("stoich",
        "Specify the Id of the stoichiometry system to use", &SteadyState::setStoich,
        &SteadyState::getStoich);
    static ReadOnlyValueFinfo<SteadyState, bool> badStoichiometry("badStoichiometry",
        "Bool: True if there is a problem with the stoichiometry",
        &SteadyState::badStoichiometry);
    static ReadOnlyValueFinfo<SteadyState, bool> isInitialized("isInitialized",
        "True if the model has been initialized successfully",
        &SteadyState::isInitialized);
    static ReadOnlyValueFinfo<SteadyState, unsigned int> nIter("nIter",
        "Number of iterations done by steady state solver", &SteadyState::getNiter);
    static ReadOnlyValueFinfo<SteadyState, string> status(
        "status", "Status of solver", &SteadyState::getStatus);
    static ValueFinfo<SteadyState, unsigned int> maxIter("maxIter",
        "Max permissible number of iterations to try before giving up",
        &SteadyState::setMaxIter, &SteadyState::getMaxIter);
    static ValueFinfo<SteadyState, double> convergenceCriterion("convergenceCriterion",
        "Fractional accuracy required to accept convergence",
        &SteadyState::setConvergenceCriterion, &SteadyState::getConvergenceCriterion);
    static ReadOnlyValueFinfo<SteadyState, unsigned int> numVarPools("numVarPools",
        "Number of variable molecules in reaction system.", &SteadyState::getNumVarPools);
    static ReadOnlyValueFinfo<SteadyState, unsigned int> rank("rank",
        "Number of independent molecules in reaction system", &SteadyState::getRank);
    static ReadOnlyValueFinfo<SteadyState, unsigned int> stateType("stateType",
    ...
    static ReadOnlyValueFinfo<SteadyState, unsigned int> nNegEigenvalues(
        "nNegEigenvalues", "Number of negative eigenvalues: indicates type of solution",
        &SteadyState::getNnegEigenvalues);
    static ReadOnlyValueFinfo<SteadyState, unsigned int> nPosEigenvalues(
        "nPosEigenvalues", "Number of positive eigenvalues: indicates type of solution",
        &SteadyState::getNposEigenvalues);
    static ReadOnlyValueFinfo<SteadyState, unsigned int> solutionStatus("solutionStatus",
    ...
    static ReadOnlyLookupValueFinfo<SteadyState, unsigned int, double> eigenvalues(
        "eigenvalues", "Eigenvalues computed for steady state",
    ...
    static string doc[] = {"Name", "SteadyState", "Author",
    ...
    static Cinfo steadyStateCinfo("SteadyState", Neutral::initCinfo(), steadyStateFinfos,
        sizeof(steadyStateFinfos) / sizeof(Finfo*), &dinfo, doc,
...
void SteadyState::setConvergenceCriterion(double value)
{
    if(value > 1e-10)
        ...
        cout << "Warning: Convergence criterion " << value << " too small. Old value "
...
double SteadyState::getEigenvalue(const unsigned int i) const
{
    ...
    cout << "Warning: SteadyState::getEigenvalue: index " << i << " out of range "
         << eigenvalues_.size() << endl;
    return 0.0;
}
...
void SteadyState::setupSSmatrix()
{
    ...
    matrix_type_ N = matrix_type_::Zero(numVarPools_, nReacs_);
    LU_            = matrix_type_::Zero(numVarPools_, nTot);
    ...
    vector<unsigned int> rowStart = Field<vector<unsigned int>>::get(stoich_, "rowStart");
    ...
    /* LU_ has an identify matrix padded on the right. Remove it to compute the
     * rank.
     */
    rank_ = myGaussianDecomposition(LU_);

    Nr_ = matrix_type_::Zero(rank_, nReacs_);

    unsigned int nConsv = numVarPools_ - rank_;
    if(nConsv == 0) {
        cerr << "SteadyState::setupSSmatrix(): Number of conserved species is "
                "0. Aborting. #pool="
             << numVarPools_ << ", rank=" << rank_ << endl;
        return;
    }
    ...
    gamma_ = matrix_type_::Zero(nConsv, numVarPools_);
...
{
    matrix_type_ J(numVarPools_, numVarPools_);

    double tot = 0.0;
    Stoich* s  = reinterpret_cast<Stoich*>(stoich_.eref().data());
    auto nVec = LookupField<unsigned int, vector<double>>::get(s->getKsolve(), "nVec", 0);
    for(unsigned int i = 0; i < numVarPools_; ++i)
        tot += nVec[i];
    ...
    for(unsigned int i = 0; i < numVarPools_; ++i) {
        ...
        Eigen::ComplexEigenSolver<matrix_type_> ces(J);
        auto eivals = ces.eigenvalues();
        // auto eivectors = ces.eigenvectors();

        eigenvalues_.clear();
        eigenvalues_.resize(numVarPools_, 0.0);

        if(Eigen::ComputationInfo::Success != ces.info()) {
            ...
            // Eigenvalues are ready. Classify state.
            nNegEigenvalues_ = 0;
            nPosEigenvalues_ = 0;
            for(unsigned int i = 0; i < numVarPools_; ++i) {
                double r = eivals(i).real();
...
void SteadyState::settle(bool forceSetup)
{
    ...
    ss_ = new NonlinearSystem(numVarPools_);

    ss_->ri.rank      = rank_;
    ss_->ri.num_reacs = nReacs_;
    ss_->ri.num_mols  = numVarPools_;
    ss_->ri.T         = T;
    ss_->ri.Nr        = Nr_;
    ss_->ri.gamma     = gamma_;
    ss_->ri.pool      = &pool_;
    ss_->ri.nVec      = LookupField<unsigned int, vector<double>>::get(ksolve, "nVec", 0);
    ss_->ri.convergenceCriterion = convergenceCriterion_;
    ...
    for(size_t i = 0; i < numVarPools_; ++i)
        init[i] = max(0.0, sqrt(ss_->ri.nVec[i]));

    ss_->initialize<vector<double>>(init);
    ...
    for(unsigned int j = 0; j < numVarPools_; ++j)
        repair[j] = ss_->ri.nVec[j];

    int status = 1;

    // Find roots . If successful, set status to 0.
    if(ss_->find_roots_gnewton(convergenceCriterion_, maxIter_))
        status = 0;

    if(status == 0 && isSolutionValid(ss_->ri.nVec)) {
        solutionStatus_ = 0;  // Good solution

        LookupField<unsigned int, vector<double>>::set(ksolve, "nVec", 0, ss_->ri.nVec);
        // Check what we set
        vector<double> t =
            LookupField<unsigned int, vector<double>>::get(ksolve, "nVec", 0);
        ...
        cout << "Warning: SteadyState iteration failed, status = " << status_
             << ", nIter = " << ss_->ri.nIter << endl;

        for(unsigned int j = 0; j < numVarPools_; j++)
            ss_->ri.nVec[j] = repair[j];
 ...
 */
void swapRows(matrix_type_& mat, unsigned int r1, unsigned int r2)
{
    ublas::vector<value_type_> temp(mat.cols());
...
int reorderRows(matrix_type_& U, int start, int leftCol)
{
    int leftMostRow = start;
    int numReacs    = U.cols() - U.rows();
    int newLeftCol  = numReacs;
    for(size_t i = start; i < U.rows(); ++i) {
...
void eliminateRowsBelow(matrix_type_& U, int start, int leftCol)
{
    int numMols  = U.rows();
    ...
    for(int i = start + 1; i < numMols; ++i) {
        ...
        if(fabs(factor) > SteadyState::EPSILON) {
            factor = factor / pivot;
            for(size_t j = leftCol + 1; j < U.cols(); ++j) {
                double x = U(i, j);
                double y = U(start, j);
                x -= y * factor;
...
unsigned int myGaussianDecomposition(matrix_type_& U)
{
    int numMols  = U.rows();
    int numReacs = U.cols() - numMols;
    int i;
 ...
 * @param S
 */
void recalcTotal(vector<double>& tot, matrix_type_& g, const double* S)
{
    assert(g.rows() == tot.size());
...
void SteadyState::randomizeInitialCondition(const Eref& me)
{
    ...
    matrix_type_ U(numConsv, numVarPools_ + numConsv);
    ...
    assert(myGaussianDecomposition(U) == numConsv);
...
void SteadyState::fitConservationRules(
    matrix_type_& U, const vector<double>& eliminatedTotal, vector<double>& y)
{
    int numConsv = total_.size();
