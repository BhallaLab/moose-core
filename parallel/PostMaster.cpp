...
PostMaster::PostMaster()
	...
	sendBufPos_( 0 ), 
	recvBuf_( 1000, 0 ), 
	donePoll_( 0 ), comm_( &MMPI::INTRA_COMM() )
{
	localNode_ = MMPI::INTRA_COMM().Get_rank(); 
	request_ = 0;
}
...
void testPostMaster()
{
	...
	MMPI::INTRA_COMM().Barrier();
	unsigned int myNode = MMPI::INTRA_COMM().Get_rank();
	unsigned int numNodes = MMPI::INTRA_COMM().Get_size();
