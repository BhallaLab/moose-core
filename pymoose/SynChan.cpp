...
{
    double tau1;
    get < double > (Element::element(id_), "tau1",tau1);
    return tau1;
}
void SynChan::__set_tau1( double tau1 )
{
    set < double > (Element::element(id_), "tau1", tau1);
}
double SynChan::__get_tau2() const
{
    double tau2;
    get < double > (Element::element(id_), "tau2",tau2);
    return tau2;
}
void SynChan::__set_tau2( double tau2 )
{
    set < double > (Element::element(id_), "tau2", tau2);
}
bool SynChan::__get_normalizeWeights() const
{
    bool normalizeWeights;
    get < bool > (Element::element(id_), "normalizeWeights",normalizeWeights);
    return normalizeWeights;
}
void SynChan::__set_normalizeWeights( bool normalizeWeights )
{
    set < bool > (Element::element(id_), "normalizeWeights", normalizeWeights);
}
...
unsigned int SynChan::__get_numSynapses() const
{
    ...
    get < unsigned int > (Element::element(id_), "numSynapses",numSynapses);
    return numSynapses;
}
void SynChan::__set_numSynapses( unsigned int numSynapses )
{
    set < unsigned int > (Element::element(id_), "numSynapses", numSynapses);
}
double SynChan::__get_weight() const
{
    double weight;
    get < double > (Element::element(id_), "weight",weight);
    return weight;
}
void SynChan::__set_weight( double weight )
{
    set < double > (Element::element(id_), "weight", weight);
}
double SynChan::__get_delay() const
{
    double delay;
    get < double > (Element::element(id_), "delay",delay);
    return delay;
}
void SynChan::__set_delay( double delay )
{
    set < double > (Element::element(id_), "delay", delay);
}
...
{
    double synapse;
    get < double > (Element::element(id_), "synapse",synapse);
    return synapse;
}
void SynChan::__set_synapse( double synapse )
{
    set < double > (Element::element(id_), "synapse", synapse);
}
double SynChan::__get_activation() const
{
    double activation;
    get < double > (Element::element(id_), "activation",activation);
    return activation;
}
void SynChan::__set_activation( double activation )
{
    set < double > (Element::element(id_), "activation", activation);
}
double SynChan::__get_modulator() const
{
    double modulator;
    get < double > (Element::element(id_), "modulator",modulator);
    return modulator;
}
void SynChan::__set_modulator( double modulator )
{
    set < double > (Element::element(id_), "modulator", modulator);
}
#endif
