...
try:
    ...
    print('[Warning] python-libsbml is not installed. SBML related functions '
            ' will be dummy. '
            )
...
def mooseReadSBML(filepath, loadpath, solver="ee"):
    ...
    if not os.path.isfile(filepath):
        print('[WARN] %s is not found ' % filepath)
        return None
    ...
    model, specInfoMap, modelAnnotaInfo, globparameterIdValue)
                                ...
                                if not errorFlag:
                                        print(msg)
                                        # Any time in the middle if SBML does
                                        # not read then I delete everything from
                                        # model level This is important as while
                                        # reading in GUI the model will show up
                                        # untill built which is not correct
