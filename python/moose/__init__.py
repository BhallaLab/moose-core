...
import moose.moose as cppmoose
...
def loadModel(filename, modelpath, solverclass="gsl"):
    ...
    if extension in [".swc", ".p"]:
        return cppmoose.loadModelInternal(filename, modelpath, "Neutral" )

    if extension in [".g", ".cspace"]:
        # only if genesis or cspace file and method != ee then only
        # mooseAddChemSolver is called.
        ret = cppmoose.loadModelInternal(filename, modelpath, "ee")
...
def le(el=None):
    ...
    if el is None:
        ...
        if not cppmoose.exists(el):
            raise ValueError('no such element')
        el = cppmoose.element(el)
    elif isinstance(el, cppmoose.vec):
        el = el[0]
...
ce = cppmoose.setCwe  # ce is a GENESIS shorthand for change element.
...
First fix that issue with SynBase or something in that line.')
    if isinstance(target, str):
        if not cppmoose.exists(target):
            raise ValueError('%s: element does not exist.' % (target))
        target = cppmoose.vec(target)
        cppmoose.syncDataHandler(target)
...
def showfield(el, field='*', showtype=False):
    ...
    if isinstance(el, str):
        ...
        el = cppmoose.element(el)
    if field == '*':
        value_field_dict = cppmoose.getFieldDict(el.className, 'valueFinfo')
...
def getfielddoc(tokens, indent=''):
    ...
    while True:
        try:
            classelement = cppmoose.element('/classes/' + classname)
...
def getmoosedoc(tokens, inherited=False):
    ...
    with closing(docstring):
        if not tokens:
            return ""
        try:
            class_element = cppmoose.element('/classes/%s' % (tokens[0]))
        ...
        if len(tokens) > 1:
            ...
            if inherited:
                mro = eval('cppmoose.%s' % (tokens[0])).mro()
                for class_ in mro[1:]:
                    if class_ == cppmoose.melement:
                        break
                    docstring.write(toUnicode(
                        '\n\n#Inherited from %s#\n' % (class_.__name__)))
                    append_finfodocs(class_.__name__, docstring, indent)
                    if class_ == cppmoose.Neutral:    # Neutral is the toplevel moose class
                        break
...
def append_finfodocs(classname, docstring, indent):
    ...
    try:
        class_element = cppmoose.element('/classes/%s' % (classname))
    except ValueError:
        raise NameError('class \'%s\' not defined.' % (classname))
    for ftype, rname in finfotypes:
        docstring.write(toUnicode('\n*%s*\n' % (rname.capitalize())))
        try:
            finfo = cppmoose.element('%s/%s' % (class_element.path, ftype))
...
def doc(arg, inherited=True, paged=True):
    ...
    if isinstance(arg, str):
        ...
        tokens = [arg.__name__]
    elif isinstance(arg, cppmoose.melement) or isinstance(arg, cppmoose.vec):
