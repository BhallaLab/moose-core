...
def findXreacs( basepath, reacType ):
    reacs = cppmoose.wildcardFind( basepath + '/##[ISA=' + reacType + 'Base]' )
    ret = []
    for i in reacs:
        reacc = findCompt( i )
...
def removeEnzFromPool( pool ):
    kids = cppmoose.wildcardFind( pool.path + "/#" )
    for i in kids:
        if i.isA[ 'EnzBase' ]:
            cppmoose.delete( i )
        elif i.isA[ 'Function' ]:
...
def proxify( reac, reacc, direction, pool, poolc ):
    reacc_elm = cppmoose.element( reacc )
    reac_elm = cppmoose.element( reac )
    # Preserve the rates which were set up for the x-compt reacn
    #cppmoose.showfield( reac )
    dupname = pool.name + '_xfer_' + cppmoose.element(poolc).name
    #print "#############", pool, dupname, poolc
    if cppmoose.exists( reacc + '/' + dupname ):
        duppool = cppmoose.element( reacc + '/' + dupname )
    else:
        # This also deals with cases where the duppool is buffered.
        duppool = cppmoose.copy(pool, reacc_elm, dupname )
    ...
    cppmoose.connect( reac, direction, duppool, 'reac' )
    #cppmoose.showfield( reac )
    #cppmoose.showmsg( reac )

def enzProxify( enz, enzc, direction, pool, poolc ):
    if enzc == poolc:
        return
    enze = cppmoose.element( enz )
...
def disconnectReactant( reacOrEnz, reactant, duppool ):
    ...
    if cppmoose.exists( infoPath ):
        info = cppmoose.element( infoPath )
    else:
        info = cppmoose.Annotator( infoPath )

    #cppmoose.le( reactant )
    notes = ""
    #cppmoose.showmsg( reacOrEnz )
    for i in outMsgs:
...
def getOldRates( msgs ):
    if len( msgs ) > 1 :
        ...
        elm = cppmoose.element( m1.split( ' ' )[0] )
        if elm.isA[ 'ReacBase' ]:
...
def restoreXreacs( basepath ):
    proxyInfo = cppmoose.wildcardFind( basepath + "/##/#_xfer_#/info" )
    for i in proxyInfo:
        msgs = i.notes.split( msgSeparator )
        oldRates = getOldRates( msgs )
        #print( "Deleting {}".format( i.parent.path ) )
        #print msgs
        cppmoose.delete( i.parent )
        for j in msgs[1:]:
            if len( j ) > 0:
                args = j.split( ' ' )
                assert( len( args ) == 4 )
                #cppmoose.showfield( args[0] )
                cppmoose.connect( args[0], args[1], args[2], args[3] )
