...
class ChannelML:
    ...
    def readChannelMLFromFile(self, filename, params={}):
        ...
        for ionConc in channelml_element.findall(
            ".//{" + self.cml + "}ion_concentration"
        ):
    ...
    def readSynapseML(self, synapseElement, units="SI units"):
        ...
        moosesynapse.tau2 = (
            float(doub_exp_syn.attrib["decay_time"]) * Tfactor
        )  # seconds
    ...
    def readChannelML(self, channelElement, params={}, units="SI units"):
        ...
        if IVrelation.attrib["cond_law"] == "ohmic":
            ...
            if concdep is not None:
                moosechannelIonDependency = moose.Mstring(
                    moosechannel.path + "/ionDependency"
                )
        ...
        for num, gate in enumerate(gates):
            ...
            for fn_element, fn_name, fn_expr in [
                ...
                if fn_element is None:
                    self.make_function(
                        fn_name, "generic", expr_string=expr_string, concdep=concdep
                    )
    ...
    def setQ10(self, q10settings):
        if "q10_factor" in q10settings.attrib:
            self.q10factor = float(q10settings.attrib["q10_factor"]) ** (
                (self.temperature - float(q10settings.attrib["experimental_temp"]))
                / 10.0
            )
    ...
    def make_cml_function(self, element, fn_name, concdep=None):
        ...
        if fn_type in ["exponential", "sigmoid", "exp_linear"]:
            ...
            expr_string = expr_string.replace("beta", "self.beta(v" + ca_name + ")")
            self.make_function(
                fn_name, fn_type, expr_string=expr_string, concdep=concdep
            )
        else:
    ...
    def make_function(self, fn_name, fn_type, **kwargs):
        ...
        if fn_type == "exponential":
            ...
            def fn(self, v: float, ca=None) -> float:
                return float(kwargs["rate"]) * math.exp(
                    (v - float(kwargs["midpoint"])) / float(kwargs["scale"])
                )

        elif fn_type == "sigmoid":

            def fn(self, v: float, ca=None) -> float:
                return float(kwargs["rate"]) / (
            ...
            def fn(self, v: float, ca=None) -> float:
                if v - float(kwargs["midpoint"]) == 0.0:
                    return float(kwargs["rate"])
                x : float = (
                    float(kwargs["rate"])
            ...
            def fn(self, v: float, ca=None) -> float:
                ...
                if "?" in expr_str:
                    ...
                    if eval(condition, {"__builtins__": None}, allowed_locals):
                        val = eval(
                            alternativeTrue, {"__builtins__": None}, allowed_locals
                        )
                    else:
                        val = eval(
                            alternativeFalse, {"__builtins__": None}, allowed_locals
                        )
                else:
                    val = eval(expr_str, {"__builtins__": None}, allowed_locals)
                if fn_name == "tau":
                    return val / float(self.q10factor)
                else:
                    return val
...
def make_new_synapse(syn_name, postcomp, syn_name_full, nml_params):
    ...
    synhandler = moose.element(
        synid.path + "/handler"
    )  # returns SimpleSynHandler or STDPSynHandler
