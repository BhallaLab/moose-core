...
class NML2Reader(object):
    ...
    def getComp(self, pop_id, cellIndex, segId):
        return moose.element(
            '%s/%s/%s/%s' %
            (self.lib.path, pop_id, cellIndex,
    ...
    def createPopulations(self):
        for pop in self.network.populations:
            ...
            for i in range(pop.size):
                print("Creating %s/%s instances of %s under %s" %
    ...
    def createInputs(self):
        for el in self.network.explicit_inputs:
            ...
            moose.connect(input, 'output', self.getComp(pop_id, i, seg_id),
                          'injectMsg')
        ...
        for il in self.network.input_lists:
            for ii in il.input:
                ...
                moose.connect(
                    input, 'output',
                    self.getComp(il.populations, ii.get_target_cell_id(),
                                 ii.get_segment_id()), 'injectMsg')
    ...
    def createMorphology(self, nmlcell, moosecell, symmetric=True):
        ...
        for segid, comp in id_to_comp.items():
            ...
            comp.x0, comp.y0, comp.z0 = (x * self.lunit
                                         for x in map(float, (p0.x, p0.y,
                                                              p0.z)))
            p1 = segment.distal
            comp.x, comp.y, comp.z = (x * self.lunit
                                      for x in map(float, (p1.x, p1.y, p1.z)))
            ...
            comp.diameter = (float(p0.diameter) +
                             float(p1.diameter)) * self.lunit / 2
            if parent:
        ...
        for sg in morphology.segment_groups:
            sg_to_segments[sg.id] = [
                id_to_segment[m.segments] for m in sg.members
            ]
        for sg in morphology.segment_groups:
    ...
    def importBiophysics(self, nmlcell, moosecell):
        ...
        if bp is None:
            print('Warning: %s in %s has no biophysical properties' %
        ...
        if species.concentration_model in self.proto_pools:
            ...
            for innerReader in self.includes.values():
                if species.concentration_model in innerReader.proto_pools:
                    proto_pool = innerReader.proto_pools[
                        species.concentration_model]
                    break
        if not proto_pool:
    ...
    def importChannelsToCell(self, nmlcell, moosecell, membrane_properties):
        ...
        for chdens in membrane_properties.channel_densities + membrane_properties.channel_density_v_shifts:
            ...
            if self.isPassiveChan(ionChannel):
                ...
                for seg in segments:
                    self.copyChannel(chdens, self.nml_segs_to_moose[seg.id],
                                     condDensity, erev)
    ...
    def importIonChannels(self, doc, vmin=-150e-3, vmax=100e-3, vdivs=5000):
        ...
        for chan in doc.ion_channel + doc.ion_channel_hhs:
            ...
            if self.verbose:
                print(self.filename, 'Created ion channel', mchan.path, 'for',
                      chan.type, chan.id)
