...
class ChannelML:
    ...
    def readSynapseML(self, synapseElement, units="SI units"):
        ...
        self.mooseSynp_graded = _moose.Mstring(self.mooseSynp.path+'/graded')
        self.mooseSynp_graded.value = 'False'
        self.mooseSynp_mgblock = _moose.Mstring(self.mooseSynp.path+'/mgblock')
    ...
    def readChannelML(self, channelElement, params={}, units="SI units"):
        ...
        if concdep is None:
            channel = _moose.HHChannel(cPath)
        else:
            channel = _moose.HHChannel2D(cPath)
        ...
        if IVrelation.attrib['cond_law'] == "ohmic":
            ...
            channelIon = _moose.Mstring(channel.path+'/ion')
            channelIon.value = IVrelation.attrib['ion']
            if concdep is not None:
                channelIonDependency = _moose.Mstring(
        ...
        if nernstnote is not None:
            ...
            if nernst_params[0] == 'Nernst':
                nernstMstring = _moose.Mstring(channel.path+'/nernst_str')
        ...
        for num,gate in enumerate(gates):
            ...
            if concdep is None:
                moosegate = _moose.HHGate( gate_path )
                ...
                moosegate = _moose.HHGate2D( gate_path )
            ...
            ## Ca dependent channel
                ...
                # the tables
                moosegate_tableA = _moose.Interpol2D(moosegate.path+'/tableA')
                ...
                moosegate_tableB = _moose.Interpol2D(moosegate.path+'/tableB')
                ## set SI values inside MOOSE
    ...
    def readIonConcML(self, ionConcElement, units="SI units"):
        ...
        caPool = _moose.CaConc(self.libraryPath+'/'+capoolName)
        ...
        if fixedPoolInfo is not None:
            ...
            caPool_phi = _moose.Mstring(caPool.path+'/phi')
