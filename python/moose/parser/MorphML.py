...
class MorphML():
    ...
    def addSegment(self, cellName, segnum, segment):
        ...
        # previous / last compartment.
            ...
            # Create a new compartment, the _moose "hsolve" method assumes
            ...
            mooseComp = _moose.Compartment(mooseComppath)
            ...
            if 'parent' in segment.attrib:
                ...
                _moose.connect(parent, 'axial', mooseComp, 'raxial')
            else:
                parent = None
            proximal = segment.find('./{'+self.mml+'}proximal')
            if proximal is None:
    ...
    def readMorphML(self, cell, params={}, lengthUnits="micrometer"):
        ...
        if cellName == 'LIF':
            self.mooseCell = _moose.LeakyIaF(self.cellPath+'/'+cellName)
            self.segDict = {}
        else:
            # using _moose Neuron class - in previous version 'Cell' class
            # Chaitanya.
            self.mooseCell = _moose.Neuron(self.cellPath+'/'+cellName)
    ...
    def addMechanism(self, mechanism, cell, cellName):
        ...
        # is in _moose.utils not _moose.neuroml.utils !
    ...
    def set_compartment_param(self, compartment, name, value, mechName):
        ...
        # spikegen being added to the compartment
            ...
            #spikegen = _moose.SpikeGen(compartment.path+'/'+value+'_spikegen')
            #_moose.connect(compartment,"VmSrc",spikegen,"Vm")
            pass
        elif mechName is not None:

            # if mechanism is not present in compartment, deep copy from library
            if not _moose.exists(compartment.path+'/'+mechName):

                # if channel does not exist in library load it from xml file
                if not _moose.exists(self.libraryPath+"/"+mechName):
                ...
                neutralObj = _moose.Neutral(self.libraryPath+"/"+mechName)

                # Ion concentration pool
                if 'CaConc' == neutralObj.className:
                    libcaconc = _moose.CaConc(self.libraryPath+"/"+mechName)

                    # deep copies the library caconc under the compartment
                    caconc = _moose.copy(libcaconc,compartment,mechName)
                    caconc = _moose.CaConc(caconc)
                    ...
                    libchannel = _moose.HHChannel2D(self.libraryPath+"/"+mechName)
                    ## deep copies the library channel under the compartment
                    channel = _moose.copy(libchannel,compartment,mechName)
                    channel = _moose.HHChannel2D(channel)
                    _moose.connect(channel,'channel',compartment,'channel')
                elif 'HHChannel' == neutralObj.className : ## HHChannel
                    libchannel = _moose.HHChannel(self.libraryPath+"/"+mechName)

                    # deep copies the library channel under the compartment
                    channel = _moose.copy(libchannel,compartment,mechName)
                    channel = _moose.HHChannel(channel)
                    _moose.connect(channel,'channel',compartment,'channel')
            # if mechanism is present in compartment, just wrap it
            else:
                neutralObj = _moose.Neutral(compartment.path+'/'+mechName)
                # Ion concentration pool
                if 'CaConc' == neutralObj.className:
                    # wraps existing channel
                    caconc = _moose.CaConc(compartment.path+'/'+mechName)
                    channel = None
                elif 'HHChannel2D' == neutralObj.className: ## HHChannel2D
                    # wraps existing channel
                    channel = _moose.HHChannel2D(compartment.path+'/'+mechName)
                elif 'HHChannel' == neutralObj.className : ## HHChannel
                    # wraps existing channel
                    channel = _moose.HHChannel(compartment.path+'/'+mechName)
            if name == 'Gbar':
