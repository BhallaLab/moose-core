...
class NetworkML(object):
    ...
    def createInputs(self):
        ...
        Create inputs as given in NML file and attach them to _moose.
        """
    ...
    def attachInputToMoose(self, inElemXml, factors, savePlot=True):
        ...
        """attachInputToMoose
        This function create StimulousTable in _moose
        ...
        if random_stim is not None:
            ...
            stim = _moose.TimeTable(stimPath)
            stim.vec = vec
            ...
            for site in target.findall(".//{"+nmu.nml_ns+"}site"):
               ...
               if cell_name == 'LIF':
                   ...
                   m = self.connectSynapse(stim, LIF)
                    ...
                    compartment = _moose.Compartment(segment_path)
                    synchan = _moose.SynChan(
                    ...
                    m = self.connectSynapse(stim, _moose.element(synchan.path+'/synapse'))
            ...
            _moose.Neutral(self.pulseGenPath)
            pulseGenPath = '{}/{}'.format(self.pulseGenPath, inputName)
            pulsegen = _moose.PulseGen(pulseGenPath)
            icClampPath = '{}/{}'.format(self.elecPath, 'iClamp')
            _moose.Neutral(icClampPath)
            iclamp = _moose.DiffAmp('{}/{}'.format(icClampPath, inputName))
    ...
    def createPopulations(self):
        ...
        for population in populations:
            ...
            if cellname == 'LIF':
                cellid = _moose.LeakyIaF(self.libraryPath+'/'+cellname)
            else:
                # added cells as a Neuron class.
                cellid = _moose.Neuron(self.libraryPath+'/'+cellname) 
    ...
    def translate_rotate(self,obj,x,y,z,ztheta): 
        for childId in obj.children:
            ...
            if childobj.className in ['Compartment','SymCompartment']:
                ...
                child = _moose.Compartment(childId)
                x0 = child.x0
                y0 = child.y0
    ...
    def getCellPath(self, populationType, instanceId):
        ...
        # Now get the path from _moose path
        path = path.path
    ...
    def addSyapseProperties(self, projection,  syn_props, source, target):
        ...
        if not _moose.exists(os.path.join(self.libraryPath, synName)):
    ...
    def connectSynapse(self, spikegen, synapse):
        ...
        spikeStim = _moose.PulseGen('%s/spike_stim' % (synapse.parent.path))
        spikeStim.delay[0] = 50.0
        spikeStim.level[0] = 1.0
        spikeStim.width[0] = 100.0
        _moose.connect(spikeStim, 'output', spikegen, 'Vm')
        m = _moose.connect(spikegen, "spikeOut"
    ...
    def connectUsingSynChan(self, synName, prePath, post_path
        ...
        postcomp = _moose.Compartment(post_path)
        ...
        if libsyn.className == 'KinSynChan' or gradedchild.value == 'True': 
            ...
            if bugs.BUG_NetworkML_500:
                ...
                synNameFull = synName
                if not _moose.exists(post_path+'/'+synNameFull):
                    synObj = self.makeNewSynapse(synName, postcomp, synNameFull)
        ...
        if gradedchild.value=='True': 
            table = _moose.Table(syn.path+"/graded_table")
            # always connect source to input - else 'cannot create message'
            # error.
            precomp = _moose.Compartment(prePath)
        ...
        # Event based synapse
            ...
            if 'file' not in prePath:
                precomp = _moose.Compartment(prePath)
                if not _moose.exists(prePath+'/IaF_spikegen'):
                    ...
                    if not _moose.exists(prePath+'/'+synName+'_spikegen'):
                        spikegen = _moose.SpikeGen(prePath+'/'+synName+'_spikegen')
                    ...
                    spikegen = _moose.SpikeGen(prePath+'/IaF_spikegen')
                ...
                # _moose.Synapse(syn.path+'/synapse') or syn.synapse Synpase is
                ...
                if not _moose.exists(tt_path):
                    ...
                    # addSpike-s etc.  can get the Synapse element by
                    # _moose.Synapse(syn.path+'/synapse') or syn.synapse Synpase
            ...
            if libsyn.className == 'KinSynChan':
                ...
                # element by _moose.Synapse(syn.path+'/synapse') or syn.synapse
    ...
    def makeNewSynapse(self, synName, postcomp, synNameFull):
        ...
        if config.disbleCopyingOfObject:
            ...
            synid = _moose.copy(_moose.Neutral(synPath), postcomp, synNameFull)
        else:
            synid = synPath
        ...
        if childmgblock.value == 'True': 
            mgblock = _moose.Mg_block(syn.path+'/mgblock')
