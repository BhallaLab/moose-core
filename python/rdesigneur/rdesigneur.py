...
class rdesigneur:
    ...
    def __init__(
        ...
        if meshLambda > 0.0:
            print(
                "Warning: meshLambda argument is deprecated. Please use "
    ...
    def _printModelStats(self):
        ...
        print(
            ...
            self.elecid.numCompartments,
            "compartments and",
            self.elecid.numSpines,
            "spines on",
            len(self.cellPortionElist),
            "compartments.",
        )
        if hasattr(self, "chemid") and len(self.chemDistrib) > 0:
            ...
            for j in moose.wildcardFind("/model/chem/##[ISA=ChemCompt]"):
                ...
                print(
                    "    | In {}, {} voxels X {} pools".format(
                        j.name, j.mesh.num, s.numAllPools
                    )
                )
    ...
    def buildModel(self, modelPath="/model"):
        if moose.exists(modelPath):
            print(
                "rdesigneur::buildModel: Build failed. Model '",
                modelPath,
                "' already exists.",
            )
            return
        ...
        for i, _func in enumerate(funcs):
            if self.benchmark:
                print(
                    "- (%02d/%d) Executing %25s" % (i + 1, len(funcs), _func.__name__),
                    end=" ",
                )
            t0 = time.time()
            try:
                ...
                print("Error: rdesigneur: model build failed: %s" % e)
    ...
    def checkAndBuildProto(self, protoType, protoVec, knownClasses, knownFileTypes):
        if len(protoVec) != 2:
            raise BuildError(
                protoType + "Proto: nargs should be 2, is " + str(len(protoVec))
            )
        ...
        if self.isKnownClassOrFile(protoVec[0], knownFileTypes):
            ...
            raise BuildError(
                protoType + "Proto: File type '" + protoVec[0] + "' not known."
            )
        return True
    ...
    def buildCellProto(self):
        ...
        for i in self.cellProtoList:
            ...
            elif self.checkAndBuildProto(
                "cell", i, ["Compartment", "SymCompartment"], ["swc", "p", "nml", "xml"]
            ):
    ...
    def buildSpineProto(self):
        for i in self.spineProtoList:
            if not self.checkAndBuildProto(
                "spine",
                i,
    ...
    def buildChemProto(self):
        for i in self.chemProtoList:
            if not self.checkAndBuildProto("chem", i, ["Pool"], ["g", "sbml", "xml"]):
    ...
    def _buildElecBallAndStick(self, args):
        parms = [
            ...
            200e-6,
            1,
        ]  # somaDia, somaLen, dendDia, dendLen, dendNumSeg
        for i in range(len(args)):
            parms[i] = args[i]
        if parms[6] <= 0:
    ...
    def _buildSynInputOnCompt(
        self, dendCompts, spineCompts, stimInfo, doPeriodic=False
    ):
        # stimInfo = [path, geomExpr, relPath, field, expr_string]
    ...
    def buildPassiveDistrib(self):
        ...
        for i in self.passiveDistrib:
            ...
            if (len(i) < 3) or (i[0] != "." and len(i) % 2 != 1):
                raise BuildError(
                    "buildPassiveDistrib: Need 3 + N*2 arguments as (path field expr [field expr]...), have {}".format(
                        len(i)
                    )
                )
        ...
        for i in self.spineDistrib:
            if len(i) >= 2:
                ...
                if len(bcKeys) > 0:  # Looks like we have an old arg str
                    print(
                        "Rdesigneur::buildSpineDistrib: Warning: Deprecated argument format.\nWill accept for now."
                    )
                    print(usageStr)
                    temp.extend(i + [""])
                elif len(i) > len(defaults):
                    print(
                        "Rdesigneur::buildSpineDistrib: Warning: too many arguments in spine definition"
                    )
                    print(usageStr)
                else:
    ...
    def _parseComptField(self, comptList, plotSpec, knownFields):
        ...
        if not field in knownFields:
            print(
                "Warning: Rdesigneur::_parseComptField: Unknown field '{}'".format(
                    field
                )
            )
            return (), ""
        ...
        if kf[0] in ["CaConcBase", "ChanBase", "NMDAChan", "VClamp"]:
            objList = self._collapseElistToPathAndClass(
                comptList, plotSpec.relpath, kf[0]
            )
            return objList, kf[1]
            ...
            if len(allObj) >= len(voxelVec):
                ...
                objList = []
                print(
                    "Warning: Rdesigneur::_parseComptField: unknown Object: '",
                    plotSpec.relpath,
                    "'",
                )
            ...
            if numPlots > 0:
                ...
                if i.mode == "wave":
                    ...
                    self.plotNames.append(
                        [tabname, i.title, k, scale, units, i.field, i.ymin, i.ymax]
                    )
                if len(i.saveFile) > 4 and i.saveFile[-4] == ".xml" or i.saveFile:
                    self.saveNames.append(
                        [tabname, len(self.saveNames), scale, units, i]
                    )
                ...
                if (
                    ...
                    if i.field == "spikeTime":
                        tabs.vec.threshold = (
                            -0.02
                        )  # Threshold for classifying Vm as a spike.
...
"""
        ...
        for i in self.plotNames:
            ...
            if i[5] == "spikeTime":
                ...
                for j in vtab:  # Plot a raster
                    y = [k] * len(j.vector)
                    plt.plot(
                        j.vector * i[3], y, linestyle="None", marker=".", markersize=10
                    )
                    plt.xlim(0, tmax)
    ...
    def initWavePlots(self, startIndex):
        ...
        for wpn in range(len(self.wavePlotNames)):
            ...
            if len(vtab) < 2:
                print(
                    "Warning: Waveplot {} abandoned, only {} points".format(
                        i[1], len(vtab)
                    )
                )
                continue
            ...
            vpts = (
                np.array(
                    [
                        [k.vector[j] for j in range(0, len(k.vector), dFrame)]
                        for k in vtab
                    ]
                ).T
                * i[3]
    ...
    def _save(self):
        for i in self.saveNames:
            ...
            if ftype == ".xml":
                ...
                print(
                    "Save format '{}' not known, please use .csv or .xml".format(ftype)
                )
    ...
    def _buildStims(self):
        ...
        for i in self.stimList:
            ...
            if i.field == "vclamp":
                ...
                stimObj3 = self._buildSynInputOnCompt(
                    dendCompts, spineCompts, i, doPeriodic=True
                )
                stimField = "setRate"
            else:
                stimObj, stimField = self._parseComptField(dendCompts, i, knownFields)
                stimObj2, stimField2 = self._parseComptField(
    ...
    def buildFromMemory(self, ePath, cPath, doCopy=False):
        ...
        self.spineList = moose.wildcardFind(
            ep + "/#spine#[ISA=CompartmentBase]," + ep + "/#head#[ISA=CompartmentBase]"
        )
    ...
    def buildFromFile(self, efile, cfile):
        ...
        self.buildFromMemory(
            self.model.path + "/tempelec", self.model.path + "/tempchem"
        )
    ...
    def _loadElec(self, efile, elecname):
        if efile[len(efile) - 2 :] == ".p":
            ...
            nm.readNeuroMLFromFile(
                efile,
                params={
                    "combineSegments": self.combineSegments,
    ...
    def _buildNeuroMesh(self):
        ...
        if "dend" in comptdict and "spine" in comptdict:
            ...
            moose.connect(
                self.dendCompt, "psdListOut", self.psdCompt, "psdList", "OneToOne"
            )
    ...
    def _buildAdaptor(
        ...
        if elecRelPath == "spine":
            ...
            elecComptList = moose.element("/model/elec").spineIdsFromCompartmentIds[
                mesh.elecComptList
            ]
        ...
        if len(elecComptList) == 0:
            raise BuildError(
                "buildAdaptor: no elec compts in elecComptList on: " + mesh.path
            )
        ...
        if not (moose.exists(chemPath)):
            raise BuildError("Error: buildAdaptor: no chem obj in " + chemPath)
...
class baseplot:
    def __init__(self, elecpath="soma", geom_expr="1", relpath=".", field="Vm"):
    ...
    def __init__(
        ...
        if len(saveFile) < 5:
            ...
            if len(f) < 2 or (f[-1] != "xml" and f[-1] != "csv"):
                raise BuildError(
                    "rplot: Filetype is '{}', must be of type .xml or .csv.".format(
                        f[-1]
                    )
                )
...
class rstim(baseplot):
    def __init__(
        self, elecpath="soma", geom_expr="1", relpath=".", field="inject", expr="0"
    ):
        baseplot.__init__(self, elecpath, geom_expr, relpath, field)
        self.expr = expr
