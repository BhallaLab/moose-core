...
def makeMoogli(rd, mooObj, args, fieldInfo):
    ...
    if mooField == "n" or mooField == "conc":
        ...
        reacSystem = moogul.MooReacSystem(
            mooObj,
            fieldInfo,
            field=mooField,
            relativeObj=relObjPath,
            valMin=ymin,
            valMax=ymax,
        )
        viewer.addDrawable(reacSystem)
    else:
        neuron = moogul.MooNeuron(
            rd.elecid,
            fieldInfo,
            field=mooField,
            relativeObj=relObjPath,
...
def displayMoogli(
    rd, _dt, _runtime, rotation=0.0, fullscreen=False, azim=0.0, elev=0.0
):
    global mooViews
