...
function do_save_plots
    str file = "squidSynGenesis.plot"

    str plot = "/axon/graphs/Vm/Vm"
    echo "/newplot" > {file}
    echo "/plotname Vm" >> {file}
    int x = {getfield {plot} npts}
    tab2file {file} {plot} xpts -table2 ypts -nentries {x - 1} 

    plot = "/axon/graphs/Vm/targetVm"
    echo "/newplot" >> {file}
    echo "/plotname targetVm" >> {file}
    tab2file {file} {plot} xpts -table2 ypts -nentries {x - 1} 

    plot = "/axon/graphs/Gk/synapse"
    echo "/newplot" >> {file}
    echo "/plotname targetGk" >> {file}
    tab2file {file} {plot} xpts -table2 ypts -nentries {x - 1} 
end
...
function add_squid_forms
    ...
    create xform /forms/control [265,585,290,260] -notitle
    ce /forms/control
    ...
    // to be applied
    create xform /forms/iclamp [560,585,290,295] -notitle
    ce /forms/iclamp
    create xlabel ilabel [10,5,255,25] -label "Current Clamp Mode"
    create xdialog baselevel [10,40,255,25] -value 0.0 -title  \
        "Base Current Level (uA)" -script  \
        "dset /pulsegen baselevel "<widget>
    create xdialog level1 [10,70,255,25] -value 0.1 -title  \
        "Pulse 1 Current (uA)" -script "dset /pulsegen level1 "<widget>
    create xdialog delay1 [10,100,255,25] -value 5 -title  \
        "Onset Delay 1 (msec)" -script "dset /pulsegen delay1 "<widget>
    create xdialog width1 [10,130,255,25] -value 40 -title  \
        "Pulse 1 Width (msec)" -script "dset /pulsegen width1 "<widget>
    create xdialog level2 [10,160,255,25] -value 0.0 -title  \
        "Pulse 2 Current (uA)" -script "dset /pulsegen level2 "<widget>
    create xdialog delay2 [10,190,255,25] -value 0 -title  \
        "Onset Delay 2 (msec)" -script "dset /pulsegen delay2 "<widget>
    create xdialog width2 [10,220,255,25] -value 0 -title  \
        "Pulse 2 Width (msec)" -script "dset /pulsegen width2 "<widget>
    create xtoggle trigmode [10,250,255,25] -title "" -script  \
    ...
    create xform /forms/exconcen [850,580,200,145] -notitle
    ce /forms/exconcen
    ...
    create xform /forms/vclamp [560,580,290,240] -notitle
    ce /forms/vclamp
